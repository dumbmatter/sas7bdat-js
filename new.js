    ...
    decompress_row(offset, length, result_length, page) {
        ...
        for (let j = 0; i < length; i++) {
            ...
            if control_byte === 0x00:
                ...
                throw new Error('unknown control byte: %s',
                                         control_byte)
            i += 1
...
class SAS7BDAT {
    constructor(path, log_level=logging.INFO, extra_time_format_strings=null, extra_date_time_format_strings=null,  extra_date_format_strings=null, skip_header=false, encoding='utf8', encoding_errors='ignore', align_correction=true) {
        ...
        this.DECOMPRESSORS = {
            RLE_COMPRESSION: this.RLEDecompressor,
            RDC_COMPRESSION: this.RDCDecompressor
        };
...
/*    __enter__() {
    ...
    __exit__(exc_type, exc_val, exc_tb) {
        """
        __exit__(*excinfo) -> null. Closes the file.
        """
        this.close()
        ...
        if (!this.cached_page) {
            for (let offset of Object.keys(offsets_to_lengths)) {
                ...
                while (skipped < (offset - this.current_file_position)) {
                    seek = offset - this.current_file_position - skipped
                    skipped += seek
                    fs.seekSync(this._file, seek, 0);
                }
                let tmp = Buffer.alloc(length);
...
console.log('header')
    ...
    convert_file(out_file, delimiter=',', step_size=100000) {
        ...
        this.logger.debug('saving as: %s', out_file)
        out_f = null
        success = true
        try:
            if out_file === '-':
            ...
            for i, line in enumerate(1) {
                ...
                try:
                    ...
                    this.logger.warn('wrote %s lines before interruption', i)
                    break
...
class ProcessingSubheader {
    constructor(parent) {
        ...
        this.ROW_COUNT_ON_MIX_PAGE_OFFSET_MULTIPLIER = 15 //
...
class SASHeader {
    constructor(parent) {
        ...
        let tmp = Buffer.alloc(this.properties.header_length - 288);
